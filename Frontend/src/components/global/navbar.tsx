...
import { FaHome, FaUser, FaCalendarAlt, FaMapMarkerAlt, FaLeaf, FaBox } from "react-icons/fa";
...
const SidebarItem: React.FC<{ item: MenuItem; isOpen: boolean }> = ({ item, isOpen }) => {
  ...
  return (
    <div>
      <Link
        ...
        hover:bg-gray-700 ${isOpen ? "w-full" : "justify-center"}`}
        onClick={(e) => {
          if (item.subItems) {
            e.preventDefault();
        ...
        <div className="flex flex-col gap-1 ml-6 mt-1">
          {item.subItems.map((subItem) => (
            <Link
              ...
              hover:bg-gray-600 text-gray-300 text-sm"
            >
